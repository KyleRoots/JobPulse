...
class CandidatesMixin:
    ...
    def get_candidate(self, candidate_id: int) -> Optional[Dict]:
        ...
        try:
            url = f"{self.base_url}entity/Candidate/{candidate_id}"
            params = {
                'fields': 'id,firstName,lastName,email,phone,mobile,address(address1,address2,city,state,zip,countryID,countryCode,countryName),status,source,occupation,companyName,skillSet,description,owner(id,firstName,lastName),customText3',
                'BhRestToken': self.rest_token
            }
