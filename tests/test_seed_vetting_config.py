...
- Production vs development defaults differ where expected
"""
...
class TestSeedVettingConfigFirstRun:
    ...
    def test_prod_defaults(self, app, vetting_config_model):
        """Production environment gets operational defaults."""
        from app import db
        from seed_database import seed_vetting_config
        ...
        with app.app_context():
            ...
            assert _val('vetting_enabled') == 'true'
            assert _val('send_recruiter_emails') == 'true'
            assert _val('embedding_similarity_threshold') == '0.45'
            assert _val('vetting_cutoff_date') == '2026-02-12 07:00:00'
