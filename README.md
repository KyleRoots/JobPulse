...
- **Env monitor + Sales Rep (Jul 28)**: Environment monitor probes `https://app.scoutgenius.ai` (auto-migrates stale lyntrix URLs — domain was never moved). Sales Rep Sync puts `BhRestToken` in query params like other Bullhorn callers (fixes ClientCorporation query HTTP 400).
