...
- **Qualified Indeed publish categories (Sep 25)**: On Qualified, industrial titles map to Warehouse, Manufacturing, Customer Service, Logistics, or Food Services instead of falling through to IT/Software Development. Titles that still do not match fall back to Manufacturing. A leftover IT/Software Development category from an earlier publish is ignored so the title can be mapped. Myticas/STSI still fall back to IT/Software Development. A category change republishes the job on the next Indeed tearsheet cycle.
