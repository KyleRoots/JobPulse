...
- **False-positive harden (Aug 10)**: First Kyle CRITICAL emails were **not** an outage — `/health` green, vetting cycle executing, screens completing. Root causes: (1) one ~102-day `processing` zombie inflight; (2) duplicate `global_settings.scheduler_last_run_*` rows (unique index missing) froze protected-job stamps at ~Jul 7 while jobs kept running; (3) SFTP warn at 60m vs 30m upload cadence. Fix: zombie→failed + dedupe/unique on `setting_key`, stall ignores ancient inflight when `completed_last_Nm>0`, scheduler miss ignores absurd frozen stamps (+ boot grace), SFTP warn → 90m. See `.agents/memory/ops-early-warning.md`.
