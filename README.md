...
- **Zip Easy Apply email integrity (Jul 23; patched Jul 26)**: Board/relay addresses (`noreply@ziprecruiter.com`, `@indeedemail.com`, …) and **owned intake mailboxes** (`apply@myticas.com`, `info@myticas.com`, …) are skipped when choosing candidate email; résumé contact wins over notification-body greets. Prevents Zip Easy Apply from collapsing every applicant onto the Bullhorn record that owns `apply@` (prod: Candidate 4380273). Zip `Great Match:` / `New candidate:` subjects still yield the applicant name.
