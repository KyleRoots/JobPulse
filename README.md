...
- **Indeed UI login retry (Aug 13)**: Transient Bullhorn Universal Login 401s (observed ~2×/month, self-recover next 5-min cycle) now retry once with a fresh session before emailing ops. Persistent failures still alert `INDEED_TEARSHEET_PUBLISH_NOTIFY_EMAIL`. **`BH_UI_*` vars are separate from REST OAuth** — rotate UI password in Railway `BH_UI_PASSWORD`, not `BULLHORN_PASSWORD` / ATS Settings.
