...
- **Indeed Unpublish fix (Jul 23)**: Native unpublish now uses CFC `method=Publish` + `operation=UNPUBLISH` (the UI’s real shape); fingerprints no longer include `dateLastModified` (stops every-cycle republish thrash).
