...
- **New requirement-spec notify (Aug 5)**: When Scout first creates an AI `JobVettingRequirements` row (not updates/regens), JobPulse emails `REQUIREMENTS_SPEC_NOTIFY_EMAIL` (default `kroots@myticas.com`) with job title/ID, Bullhorn deep link, and an interpreted-requirements excerpt for sanity-check vs the JD. Fail-soft; toggle with `REQUIREMENTS_SPEC_NOTIFY_ENABLED` (default ON).
