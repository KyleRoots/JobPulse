...
- **Qualified Staffing Railway scaffold (Sep 17)**: Second Scout deploy path for the Qualified Staffing Bullhorn corp (`JobPulse-Qualified` + `Postgres-Qualified` in `scout-genius`). Isolated DB/mailbox/API user from Myticas/STSI. `SCOUT_TENANT=qualified_staffing` selects Qualified feed filenames/publisher/apply host (tearsheet IDs empty until mapped; uploads skip until then). Apply branding on `qualified.scoutgenius.ai`. Ops checklist: `docs/qualified-staffing-deploy.md`.
