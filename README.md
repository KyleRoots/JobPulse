...
- **Qualified Indeed campaign tags (Sep 25)**: Columbus-Key now publishes as `#INDCol` (same as Columbus). Columbus-KIT stays `#INDCon`. Chattanooga is `#INDChat`, Grand Rapids is `#INDGrand`, and Katy is `#INDKat`. The Columbus-Key republish rewrote that tag on the six live descriptions.
...
- **Qualified Indeed campaign tags (Sep 21)**: On `SCOUT_TENANT=qualified_staffing`, native Indeed publish maps Bullhorn `correlatedCustomText1` (department) to ATSA tags (e.g. Appleton → `#IND-WH`, Internal → `#INDWin`, Southfield → `#INDLiv`, Columbus-Key → `#INDCol`, Columbus-KIT → `#INDCon`, Port Huron → `#INDMary`), strips leftover `#INDShow` / unhashed legacy markers, and appends `   {tag}` (three spaces). Unmapped or blank department skips publish with a clear reason. Myticas/STSI still use `#INDShow` on tearsheet 1640.
...
- **Telecom/satellite employer boost (Aug 26)**: Scout job config now has a second +5 checkbox beside consulting prestige, for major telecom and satellite employers. Independent toggles, same location gate, no stacking. Recognition is a fixed list (not an AI guess), including Canadian names such as Telesat, Bell, Rogers, TELUS, Shaw, Videotron, and MDA Space. Railway healthcheck uses `/ping`; gunicorn binds before scheduler/Bullhorn work so deploys are not blocked by boot locks.
...
- **Myticas client onboarding notify (Aug 19)**: Observe-only. First **Client Submission (Sendout)** or **Interview** on a **new** Myticas company (created in Bullhorn at or after go-live) emails Accounting (`accounting@myticas.com`), CCs the Sales Rep, and BCCs `kroots@myticas.com`, attaching the Ottawa new-client checklist. Existing companies are not notified. Status must be Qualified / Proposal / Negotiation / Active Account; blank Type is OK; Vendor / MSP / Former Client skipped. **STSI Sales Reps** (`@stsigroup.com`, or department containing STSI) are skipped so Myticas Accounting is not notified for STSI customers. Once per company. Toggle with `CLIENT_OB_NOTIFY_LIVE`. No Bullhorn writes.
...
- **False-positive harden (Aug 10)**: First Kyle CRITICAL emails were **not** an outage — `/health` green, vetting cycle executing, screens completing. Root causes: (1) one ~102-day `processing` zombie inflight; (2) `global_settings` lacked PRIMARY KEY (duplicate `id`s across keys) + duplicate `setting_key`s → scheduler listener `StaleDataError` froze stamps at ~Jul 7 while jobs kept running; (3) SFTP warn at 60m vs 30m upload cadence. Fix: zombie→failed; restore PK + unique; `set_value` updates by key via SQL; stall ignores ancient inflight when completions flow; miss ignores absurd frozen stamps; SFTP warn → 90m. See `.agents/memory/ops-early-warning.md`.
- **Failed Scout note self-heal (Aug 10)**: Rare completed screens could leave `note_created=False` (Bullhorn showed only AI Resume Summary; self-screen cooldown + `ParsedEmail.vetted_at` then blocked natural retry — e.g. [Terry Vallo](https://cls45.bullhornstaffing.com/BullhornStaffing/OpenWindow.cfm?Entity=Candidate&id=4674305)). Scheduled `retry_failed_screening_notes` (every 10 min) rewrites missing Scout notes and retries recruiter notifications (including Location Review) for screens in the last 24h only — older historical `notifications_sent=False` rows are skipped to avoid email spam. Location Review / Prestige sends now flip `notifications_sent` on the vetting log (same as Qualified). Manual `/screening/retry-failed-notes` now notifies beyond Qualified-only.
...
- **Deploy healthcheck hang fix (Aug 10)**: Boot `run_schema_migrations` no longer runs unconditional `DROP CONSTRAINT IF EXISTS` / `CREATE INDEX IF NOT EXISTS` every start when the Task #100 unique swaps are already done. Catalog-check first + `lock_timeout` — those DDL locks were hanging Railway `/health` behind idle-in-transaction sessions.
- **Screening output/note compression (Aug 10)**: Further cut gpt-5.4 **output** tokens without changing scores. CLEAR-REJECT / related-&lt;70 prose tightened (≤25-word summary, one decisive gap, ≤8-word skills/experience). Compact caps: applied/escalate **1800** (was 2200), related brief **1200** (was 1400). NOT RECOMMENDED OTHER TOP MATCHES: one full related write-up; additional near-misses only if ≥65% as a complete one-liner. Applied near-miss (≥60) and Qualified write-ups unchanged. Terra shadow stays off (`SCREENING_AB_SHADOW_ENABLED=false`).
...
- **Alexa Nasrallah LinkedIn seat (Aug 10)**: Bullhorn display name `Alexa Nasrallah` → `#LI-AN1` (seed + monitoring fallback); LinkedIn seat Tag must also be `#LI-AN1` for XML attribution
...
- **Indeed UI login retry (Aug 13; hardened Aug 27)**: Transient Bullhorn Universal Login 401s (observed ~2×/month, self-recover next 5-min cycle) retry up to 3 times with a fresh session. Failure emails are cooldown-gated (30 min, same subject) so a one-cycle blip does not page ops every 5 minutes. Persistent failures still alert `INDEED_TEARSHEET_PUBLISH_NOTIFY_EMAIL`. **`BH_UI_*` vars are separate from REST OAuth** — rotate UI password in Railway `BH_UI_PASSWORD`, not `BULLHORN_PASSWORD` / ATS Settings.
...
- **OTHER TOP MATCHES note hygiene (Jul 31; tightened Aug 10)**: Not-recommended notes keep a full write-up for the **top 1** related role; additional related roles below 65% are omitted (no mid-sentence `…` gap cuts — Saitharun / 4673413). Near-misses (≥65) use one complete gap clause only. Prompt brevity also forbids unfinished sentences.
...
- **Terra shadow token ceiling (Aug 3)**: Compact shadow calls use `max_completion_tokens=4000` by default (prod escalate stays at 1800; related brief 1200) so `gpt-5.6-terra` reasoning no longer truncates to `empty_response (finish=length)`. Tune via `SCREENING_AB_SHADOW_MAX_COMPLETION_TOKENS` (code default is enough — no required Railway var). Canary remains off in production.
