...
- **Qualified Indeed campaign tags (Sep 21)**: On `SCOUT_TENANT=qualified_staffing`, native Indeed publish maps Bullhorn `correlatedCustomText1` (department) to ATSA tags (e.g. Appleton → `#IND-WH`, Internal → `#INDWin`, Southfield → `#INDLiv`, Columbus-Key and Columbus-KIT → `#INDCol`, Port Huron → `#INDMary`), strips leftover `#INDShow` / unhashed legacy markers, and appends `   {tag}` (three spaces). Unmapped or blank department skips publish with a clear reason. Myticas/STSI still use `#INDShow` on tearsheet 1640.
