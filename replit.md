...
### Active — Mailbox-Pull Ingestion + Résumé Recovery (EMERGENCY, 2026-06-04)
- **Why**: Replit's GCE load balancer truncates inbound POST bodies (≤4096B), so the SendGrid Inbound Parse webhook silently lost applicant emails. Fix = PULL applicant emails from the `apply@myticas.com` O365 shared mailbox via Microsoft Graph into the EXISTING `process_email` pipeline (DB-flag `mailbox_pull_enabled`, toggle in `/vetting/settings`, no republish).
- **Graph attachment bug — FIXED 2026-06-04**: `get_attachments` used `$select=...,contentBytes`, but the attachments collection is polymorphic so Graph returned **400** (fail-soft → `[]`), so ~10 applicants (ParsedEmail ids 11503–11512, since 2026-06-04 07:24:20 UTC) ingested to Bullhorn WITHOUT their résumé (status=completed, bullhorn_candidate_id set, resume_file_id/filename NULL). Fixed by removing `$select`. User set `mailbox_pull_enabled=false` to stop the bleed.
- **Résumé Recovery tool — SHIPPED this batch, pending ONE republish + run**: new **"Recover Missing Résumés"** button on `/vetting/settings` (24h/48h/72h/7d selector, default 72h) → re-fetches each affected applicant's original email by `internetMessageId`, re-parses + attaches the résumé to the **EXISTING** Bullhorn candidate (NEVER creates a candidate/submission), sets resume_file_id, then resets them for re-vet + enqueues one cycle. Idempotent: skips rows that already have a résumé (`resume_file_id IS NULL` gate), commits the résumé marker on its own commit right after upload, and serialized by a `resume_recovery_in_progress` single-flight flag. Dev boot clean, compiles, Jinja parses, architect-reviewed (idempotency/concurrency fixes applied).
- **Operator steps**: (1) **republish** (ships the Graph `$select` fix + the recovery tool together), (2) flip `mailbox_pull_enabled=true` so steady polling resumes, (3) click **Recover Missing Résumés** (72h covers the 2026-06-04 incident window). **Post-run verify**: the 10 rows now have resume_file_id set; the candidates get re-vetted; spot-check one Bullhorn candidate has exactly ONE résumé file (no duplicate); confirm no new "Recover" run double-uploads (re-running should report recovered=0).
