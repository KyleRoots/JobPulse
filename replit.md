...
- **Job Eligibility — Single Source of Truth**: `utils/job_status.py` exposes `INELIGIBLE_STATUSES` and `is_job_eligible(job)`. A job is ineligible if EITHER `isOpen` is falsy OR its status is in the ineligible set (Filled, Archive, Closed, Covered, Lost variants, etc.). The applied-job injection path (`screening/detection.py::_fetch_applied_job`) uses this helper so it can no longer drift from the dashboard/feed filters. Fixes a logic bug where half-closed jobs (`isOpen=false` left with stale `status='Accepting Candidates'`) were being injected and screened — observed as job 31896 receiving 21 spurious vetting attempts in 24h.
