...
### Active — June 1, 2026 Cost Lab (TURN-ON EXECUTED 2026-06-01 — 48-72h EVAL PENDING)

> **TURN-ON RESULTS (2026-06-01, verified via prod read-replica)**: Screening re-enabled + both audit secrets republished. **Both audits firing cleanly** — `screening_ab_log` since 2026-06-01: `|strict` (schema audit) 105 rows, `|cache_optimized` (L2 cache audit) 33 rows. The playbook's "strong prior" zero-rows failure mode did NOT materialize. **Step-8 telemetry gap RESOLVED**: `screening.scoring` ($29.19, 1059 rows) and `screening.scoring.shadow` ($3.78, 138 rows) are writing to `openai_call_log` again (were zero since 2026-05-11) — telemetry now captures the dominant cost surface. **Cost standing**: ~$35 telemetry-true in first ~13.5h → ~$62/24h projected (GREEN, well under $130/24h investigation threshold); shadow ~$6.7/24h projected (under $15/24h killswitch, but running a touch hotter than the original schema-only $5-10/48-72h estimate now that BOTH audits run — keep an eye on it). **Fraud detection** (enabled this turn): 160 clear / 4 review / 0 high-risk, ~2.4% review rate, no false-positive storm. **STILL PENDING**: 48-72h cutover evaluation (step 7) against the 6-criterion playbooks; user calendar-reminded to revisit ~2026-06-08 for `fraud_bullhorn_note_enabled` enablement after threshold calibration. **Note**: `🔎 schema-audit boot-check:` log line was NOT retrievable from the fetchable deployment-log window (rolled off) — DB row counts are the authoritative confirmation and supersede it.
...
- **🆕 Fraud / Fake-Candidate Detection Phase 1 — SHIPPED, GATED OFF (2026-05-29)**: Deterministic ($0 AI) advisory layer built and merged. All four config flags default OFF/safe (`fraud_detection_enabled=false`, `fraud_bullhorn_note_enabled=false`, thresholds 40/75). **Safe to enable during the budget freeze — zero OpenAI cost** (all signals are deterministic; near-dup reuses cached embeddings only, never calls the API). **Surfacing (3 channels)**: (a) recruiter-portal badge + audit table, (b) vendor-neutral Bullhorn note on High-Risk only, and (c) **recruiter notification emails** — an advisory banner (band + score + contributing signals) now renders on all three recruiter-email surfaces (qualified, prestige-review, location-review) for `review`/`high_risk` bands, gated by the same `fraud_detection_enabled` flag (no separate toggle), fully fail-soft so it can never block a send. **Note**: cross-candidate signals (identity/resume reuse, near-dup, velocity) compare against candidates that have flowed through Scout Genius vetting (local Postgres history), NOT the full Bullhorn DB; phone-reuse-across-identities is inactive (no phone column persisted) — only email reuse is live. These are Phase 1 coverage boundaries, not defects. **Turn-on checklist when ready**: (1) flip `fraud_detection_enabled=true` in `/vetting/settings`; (2) leave Bullhorn notes OFF for an initial observation window, watch the recruiter-portal badges + `candidate_fraud_assessment` rows to calibrate thresholds against real candidates; (3) only after thresholds look right, flip `fraud_bullhorn_note_enabled=true` so High-Risk candidates get the vendor-neutral Bullhorn note. **Watch-items**: confirm no badge false-positive storms (tune `fraud_high_risk_threshold` up if so); confirm advisory contract holds (screening must NEVER skip/block — engine uses isolated DB sessions + fail-soft hook, verified by tests). Phase 2 (light-AI signals, repeat-offender alerting, auto-gating) explicitly deferred.
- **MAY 2026 BUDGET FREEZE — ENDED 2026-06-01**: OpenAI monthly cap of $2,450 hit on 2026-05-19; screening disabled by user through end of May, inbound email parsing kept active. **Freeze lifted 2026-06-01** — screening re-enabled, both audits + fraud detection turned on (see June 1 Cost Lab turn-on results above). New June billing baseline now accruing.
- **AI cost trend** (post-turn-on 2026-06-01): ~$35 telemetry-true in first ~13.5h → **~$62/24h projected, GREEN** (well under $130/24h investigation threshold). Top spender `screening.scoring` $29.19 (83%); `screening.scoring.shadow` $3.78 (both audits combined). High prompt-cache hit rate observed (~79% cached input tokens on `screening.scoring`). **TELEMETRY GAP RESOLVED**: `screening.scoring` + `.shadow` (zero rows in `openai_call_log` since 2026-05-11) resumed writing on the June 1 re-enable/republish — telemetry now captures the dominant surface, so the prior telemetry-vs-billing reconciliation premise has largely closed (confirm against the actual June OpenAI bill before declaring the historic 6-7x multiplier gone).
...
- **Legacy `.doc` resume extraction — FIXED (2026-05-30, pending prod verify)**: Genuine OLE2 `.doc` resumes used to extract zero text in production because the pipeline relied on the `antiword` system binary, which works in dev but reliably EIOs (`OSError errno=5`) in the deployed Nix runtime. **Fix shipped**: added a pure-Python `olefile`-based extractor (`_try_olefile_doc` in `utils/doc_extraction.py`) that reads the WordDocument stream and parses the FIB + piece table directly — identical behaviour in dev and prod. It now runs **FIRST** in the OLE2 branch (antiword kept only as a secondary fallback), so the fragile binary is no longer on the critical path. A printable-ratio guard (≥0.80) ensures binary garbage returns `None` instead of polluting candidate records. Validated against 4 real OLE2 fixtures (Apache POI test-data, Apache-2.0) in `tests/fixtures/` with antiword as ground-truth oracle; 29 tests green in `tests/test_doc_extraction.py`. `olefile` added to `requirements.txt`. **Post-deploy verify**: after next publish, confirm a real `.doc` resume parses (log line `✅ olefile extracted N chars`) and watch for any `olefile .doc parse failed` debug lines on real candidate uploads.
