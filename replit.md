...
- **Missing Employment Dates Gate (Gate 4)**: Post-processing gate in `candidate_vetting_service.py` that detects when a resume contains no employment date references (year ranges, month/year combos, "since YYYY", etc.) using `_has_employment_dates()` regex helper. If the job requires 2+ years of experience AND no dates are present, score is capped at 70 (or 65 for 5+ year requirements) and `gaps_identified` includes "Employment dates absent — years of experience cannot be verified for this role." Jobs with no explicit year requirements are not penalised. AI prompt also instructs GPT-4o to default `total_professional_years` to 2.0 when dates are absent, rather than a high value.
