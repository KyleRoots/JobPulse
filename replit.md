...
- **Screening Skip Gates — Loop Killer Batch (May 2026)**: Three layered gates added to `screening/dedup.py` and `screening/note_builder.py` to stop the duplicate-vetting loop bug (Hemanth-style 3 cycles in 8 min).
  (1) **Self-Screen Cooldown Gate** — new `_self_screen_cooldown_active(candidate_id)` runs FIRST inside `_should_skip_candidate` and blocks ANY re-screen of a candidate within `self_screen_cooldown_minutes` of their most recent vetting_log row, regardless of `applied_job_id`. Closes the gap where the existing 24h same-job dedup misses cycle re-detections that hit different tearsheet jobs each pass. Configurable 0–720 min (default 60, 0 disables). Bypass works in practice: Sandbox runs its own analysis flow outside `_should_skip_candidate`; Quality-Auditor revets call `reset_candidate_for_revet` which deletes prior vetting_log + CandidateJobMatch rows so the next detection pass sees no recent activity and proceeds.
  (2) **Recruiter-Decisioned Full-Skip Gate** — new `_is_paused_by_recruiter_decision(bullhorn, candidate_id, applied_job_id)` skips the entire re-screen for a (candidate × job) pair when the pair has been screened before (proven by `CandidateJobMatch` row) AND a human-authored Bullhorn note exists dated AFTER the most recent "Scout Screen" note on the candidate. Cross-role discovery preserved: brand-new (candidate × job) pairs always proceed. Killswitch `recruiter_decision_skip_enabled` (default true). Excludes the configured `api_user_ids` set + the auth user when classifying note authorship. Fail-open on Bullhorn / DB errors.
  (3) **Note-Dedupe Rejection Counter** — module-level `_DEDUPE_REJECTION_COUNTER` in `screening/note_builder.py` increments on every dedupe-blocked note creation; structured log line `event=note_dedupe_blocked counter=N vetting_log_id=X candidate_id=Y existing_actions=[...]` surfaces visibility into how often the safeguard fires (Sentry/Datadog grep target).
  Two new VettingConfig defaults seeded (`self_screen_cooldown_minutes`, `recruiter_decision_skip_enabled`); admin form at `/vetting/settings` exposes both with cooldown validation (0–720 cap) and audit-log payload coverage. 17 new tests in `tests/test_screening_skip_gates.py` (cooldown bypass, killswitch, brand-new-job pass-through, human-note-after-screen skip, api_user exclusion, fail-open paths). Targets ~70–80% reduction in redundant vetting cycles and Bullhorn note churn.
- **Recruiter Email Enhancements — Job-Aware Subject + Resume Attachment (May 2026)**: `screening/notification.py` upgraded with two recruiter-UX wins per internal user request. (1) **Job-aware subject (Option A)** — new module-level `_build_recruiter_subject(candidate_name, matches)` replaces the legacy `🎯 Qualified Candidate Alert: {name}` subject with `Scout: {Name} — {Top Job Title} (Job #{ID})` for single matches and `… +{N-1} more` for multi-match alerts. Top match = highest `match_score` (stable sort, missing scores treated as 0). Falls back to legacy format when matches list is empty; handles missing job titles ("Position") and missing job IDs (omits "Job #" suffix); blank candidate names default to "Candidate". (2) **Best-effort resume attachment** — new `_fetch_resume_attachment(candidate_id, candidate_name)` mixin method calls existing `get_candidate_resume()`, applies a 10MB cap (`_RESUME_ATTACHMENT_MAX_BYTES`), and builds a SendGrid-ready dict with `_safe_resume_filename` (sanitized `{Candidate_Name}_Resume.{ext}`) and `_resume_content_type` (extension-based MIME inference: pdf/doc/docx/rtf/txt/odt → octet-stream fallback). Fully fail-open: any Bullhorn fetch error, missing file, or oversize resume returns `None` and the email sends without the attachment — recruiter notification always wins over convenience attachment. `_send_recruiter_email` signature gained `attachments: Optional[list] = None` and threads it through to `email_service.send_html_email`. 22 new tests in `tests/test_recruiter_email_enhancements.py` (subject sort/edge cases, filename sanitization, content-type inference, fail-open paths, size-cap boundary).
