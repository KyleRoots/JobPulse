...
- **Zero-Score Verification**: When a candidate scores 0% on ALL jobs, an automatic re-verification step runs on the top 3 jobs using gpt-4.1-mini. The AI reviews the original 0% assessment and confirms whether it's truly zero relevance or if some minimal score (2-15%) is warranted for transferable skills/partial overlap. Revised scores update the match records; confirmed 0% scores are left unchanged. This prevents false 0% scores while still allowing genuine non-fits.
