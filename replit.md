...
- **Cost standing**: ~$62/24h projected (GREEN, under $130/24h investigation threshold); shadow ~$6.7/24h (under $15/24h killswitch but running hotter now both audits run — watch it). Top spender `screening.scoring` (~83%), ~79% prompt-cache hit rate.
- **Still pending**: (1) **48-72h cutover eval (~June 3-4)** against the 6-criterion playbooks (schema in `.local/tasks/output-token-diet.md`; L2 cache in `docs/archive-2026-05.md`); (2) **billing reconciliation** — confirm telemetry vs the actual June OpenAI bill before declaring the historic 6-7x multiplier gone.
