...
- **Mini-model cheap-first routing — EVALUATED 2026-06-05, viable, build pending user go (`Power`)**: the gpt-4.1-mini A/B shadow (gated by `SCREENING_AB_SHADOW_ENABLED=true` + `SHADOW_LOGGING_DISABLED=false` — intentionally still ON as the data source; this is NOT one of the audit flags, which are off) has logged 6,302 head-to-head scorings vs gpt-5.4 since 2026-05-14. Finding: **97.8% qualify/reject agreement**; mini scores +8.5 higher on average (generous → rarely false-rejects); base rate only ~0.7% qualify (43/6,302). A cheap-first gate — mini scores everyone; if mini < 40 auto-reject and skip gpt-5.4; otherwise escalate to gpt-5.4 as the authoritative scorer — routes **~74.5% cheap with 0/43 qualified lost**. Est. ~62% off the scoring line → **~$2,000–2,600/mo ongoing** (higher transiently while backlog drains). Plan: build behind a flag with a **canary-confirm phase** (run the gate but still score gpt-5.4 in parallel a few days, confirm 0 live false-negatives) before flipping to savings mode; keep mini A/B on as monitor; retire shadow after cutover. gpt-5.4 still makes every real qualification decision. Design detail in `.agents/memory/cheaper-model-scoring-experiment.md`.
