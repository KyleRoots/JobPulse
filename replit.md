...
- **Scout Prospector**: AI-powered B2B sales prospecting tool for staffing recruiters. Uses OpenAI Responses API with `web_search_preview` tool to discover ideal client companies based on configurable Ideal Client Profiles (ICPs). Features: ICP CRUD with industry/size/geography/job-type/hiring-signal criteria (with Select All/Deselect All for checkboxes), AI Suggest Criteria (auto-fills Steps 2–4 from description with confirmation toast), GPT-powered web research sessions, prospect management with qualification scoring and status tracking (new/hot/warm/cold/contacted/not_interested), CSV export, prospect detail views with key contacts and source URLs, and branded help guide at `/scout-prospector/guide`. Header toggle pill included for non-admin multi-module users. Settings page displays "Prospector" with crosshairs icon. Files: `scout_prospector_service.py`, `routes/scout_prospector.py`, `templates/scout_prospector*.html`, `templates/scout_prospector_guide.html`. Theme accent: `#3b82f6` (royal blue), `data-module="prospector"`, `active_page="prospector"`. Models: `ProspectorProfile`, `ProspectorRun`, `Prospect`.
