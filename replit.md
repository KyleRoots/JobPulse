...
- **Legacy `.doc` resume extraction broken in PRODUCTION (logged 2026-05-30)**: Genuine OLE2 `.doc` resumes (NOT `.docx`) extract zero text in the deployed app. The pipeline relies on the `antiword` system binary, which **works in dev but reliably fails in production with `OSError errno=5 (EIO)`** — persists even after a fresh publish. Diagnosed as a deployment-runtime/Nix-store issue, NOT a missing library (`ldd antiword` shows only glibc, resolves in dev) and NOT a code bug — `utils/doc_extraction.py` is fail-soft (clean `None`, never crashes), so this is **lost coverage, not an outage**. Candidates submitting `.doc` resumes flow through with no parsed contact info / no vetting text.
  - **Future task candidate** (Economy-tier): remove the hard `antiword` dependency by adding a pure-Python OLE2 fallback. (1) install `olefile` (not currently installed); (2) add `_try_olefile_doc()` to `utils/doc_extraction.py` — read the `WordDocument` stream, parse the FIB + piece table (CLX/plcfpcd in `0Table`/`1Table`) per the standard antiword/Apache-POI algorithm; (3) **printable-ratio guard** so non-text binaries return `None` rather than polluting candidate records (downside strictly bounded — never worse than today); (4) wire in as the step after antiword in the OLE2 branch. **Why deferred (not done inline 2026-05-30)**: no real `.doc` fixture available and no LibreOffice/`soffice` in this env to generate one, so a hand-rolled piece-table parser can't be safely validated — shipping it untested into the recruiting pipeline risks emitting garbage into candidate records. Task must ship a real `.doc` fixture + `tests/test_doc_extraction.py` coverage (simple + fast-saved + guard-rejects-binary).
