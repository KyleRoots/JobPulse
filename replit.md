...
- **🛑 June budget pause (2026-06-08): screening OFF for the rest of June** — June MTD (~$2,800 telemetry) exceeded the ~$2,500 cap. `vetting_enabled=false` + `screening_audit_enabled=false` (`/vetting/settings`); A/B shadow retired (`SHADOW_LOGGING_DISABLED=true` + republish). Verified 2026-06-08: scoring spend cliff **$36/hr → $0.51/hr**; only cheap inbound parsing continues (~$12–20/day) with `mailbox_pull_enabled=true` (applicants still reach Bullhorn, no scoring). **Resume 2026-07-01** — see the July-1 Re-enable Checklist below.
...
- **PandoLogic feed source attribution — deployed 2026-06-09, pando-path verify pending**: inbound apply-form mapping keys off `feed=pando` (not the referrer/source string) → Bullhorn source="Corporate Website" + owner=PandoLogic API user (`VettingConfig.pandologic_api_user_id`); genuine LinkedIn/Indeed/Facebook referrers (no feed=pando) keep their true source. Only affects NEW applications (no retro-fix of ~900 already mislabeled). Non-pando path already confirmed in prod (a live LinkedIn applicant kept "LinkedIn Job Board"). **Verify**: the first real `PandoLogic feed detected (feed=...): source -> 'Corporate Website', owner -> 4582033` log line + matching Bullhorn candidate. `tests/test_pando_source_attribution.py` (21). One-line switch to "Vendor/3rd Party" via `_InboundCore.PANDO_FEED_SOURCE`. (Supersedes the referrer-based attribution shipped 2026-06-05, where PandoLogic referrers fell through; that item verified + archived.)
...
1. `/vetting/settings` → enable `vetting_enabled` (+ `screening_audit_enabled` if wanted) → Save → click **Start Fresh** — resets BOTH `vetting_cutoff_date` AND `last_run_timestamp`, so only net-new is scored (no June backlog from the inbound OR Bullhorn-direct path; `tests/test_start_fresh_cutover.py`).
