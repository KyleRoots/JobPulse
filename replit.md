...
- **🛑 June budget pause (2026-06-08): screening OFF for the rest of June** — June MTD (~$2,800 telemetry) exceeded the ~$2,500 cap. Set `vetting_enabled=false` + `screening_audit_enabled=false` (`/vetting/settings`); A/B shadow retired (`SHADOW_LOGGING_DISABLED=true` + republish). Verified 2026-06-08 ~13:30: scoring spend cliff **$36/hr → $0.51/hr** at ~12:42; only cheap inbound parsing continues (~$12–20/day) with `mailbox_pull_enabled=true` (applicants still reach Bullhorn, no scoring). **Resume 2026-07-01** with Enforce + retired shadow already baked in (`screening_routing_mode=enforce`, `layer2_model=gpt-4.1-mini`). **July-1 procedure:** `/vetting/settings` → enable `vetting_enabled` (+ `screening_audit_enabled` if wanted) → Save → click **Start Fresh** — now resets BOTH `vetting_cutoff_date` AND `last_run_timestamp`, so only net-new is scored (no June backlog from the inbound OR Bullhorn-direct path; `tests/test_start_fresh_cutover.py`).
...
- **Backlog**: scoring ~7,200–8,400/day vs ~2,400–3,000 baseline — post-outage (May 11–Jun 1) catch-up draining, flat not spiking. Billing-true ≈ $554/day now (telemetry × 1.6); steady-state ≈ $110–140/day. **Re-measure steady-state deferred to the 2026-07-01 re-enable (screening paused for June).**
...
- **🗓️ Jun 8–9 Checkpoint Agenda (Enforce validation gate — agent must raise proactively at next session):** Enforce has been LIVE since ~2026-06-05 ~20:38 UTC. **UPDATE 2026-06-08 — screening PAUSED for the rest of June (budget); see the 🛑 June budget pause banner above. (1) RETIRE A/B shadow ✅ DONE (off, verified $0/day). (2)–(4) require live scoring → deferred to the 2026-07-01 re-enable. (5) replit.md cleanup: full archive still deferred to July.** Interim read 2026-06-07 = healthy — scoring saving **~$94/day (~$2,800/mo)** vs all-gpt-5.4, qualify rate holding (~1%/day, notifications still flowing), **0** `🚨 CANARY false-negative` + 0 `AI analysis error`/numeric-crash lines, cheap-reject path firing (`💸 Cheap-first reject`). Checkpoint actions, in priority order: **(1) RETIRE the A/B shadow** — once the gate confirms clean (0 false-negative + qualify-rate parity over the full window), flip `SCREENING_AB_SHADOW_ENABLED=false` (or `SHADOW_LOGGING_DISABLED=true`) + republish to recover **~$1,950/mo** of pure monitoring overhead (measured ~$65/day on `screening.scoring.shadow`). NOTE: the shadow is the SOLE false-negative detector in Enforce, so retire ONLY after the gate passes — do not turn it off early. (2) Confirm 0 false-negative lines AND qualify-rate parity across the full window (not just a spot day). (3) Re-measure scoring steady-state (ties to the Backlog item below). (4) Skip rate currently **~46%** (below the ~74.5% design target) because a few mid-band candidates are re-scored repeatedly → all escalate; reinforces the **Re-vet dedupe** opportunity above (would compound the savings). **(5) replit.md cleanup (do LAST, only once 1–4 are confirmed + confident for long-term):** archive the routing/cost-optimization play-by-play into `docs/archive-2026-06.md` (like the earlier batches) and trim this section to one-line pointers — keep current state, drop the historical detail.
...
- **Note toggles** (in `/vetting/settings`, both OFF during observation): `fraud_bullhorn_note_enabled` (master, High-Risk only by default), `fraud_note_all_bands_enabled` (widens to all bands when master on). **Next step**: enable High-Risk notes once thresholds look right — deferred to the 2026-07-01 re-enable (fraud assessment runs inside the vetting pipeline, which is paused for June).
