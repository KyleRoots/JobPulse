...
- **Duplicate Note Cleanup Tool**: Database-wide Automation Hub tool that finds and removes duplicate notes on candidate records. Duplicates = same author + identical comments within a configurable time window (default 60 min). Keeps newest, soft-deletes older copies. Action type dropdown (32 types) with smart candidate sourcing: Owner Reassignment → cooldown table, AI Vetter → vetting logs, All/other → paginated Bullhorn search (full database). Runs in background for large scans (22K+ candidates). Has preview (dry-run) and live execution modes.
- **Owner Reassignment — Entity Association Note Lookup (Bug #5, May 2026)**: `_find_first_human_interactor` previously queried Bullhorn's `search/Note?query=personReference.id:X` index, which returned `total=0` in production for candidates whose notes were clearly visible in the UI (ownership flip never fired). Switched to the canonical `entity/Candidate/{id}?fields=notes(id,commentingPerson(...),dateAdded,action)` to-many association lookup, which reads the live association table and is robust to whichever linkage (`personReference` for API-created notes vs `candidates` for UI-created notes) the note creator populated. The cooldown buster (`_find_cooldown_busters_via_notes`) now also matches notes via EITHER `personReference.id` OR any `candidates[].id` for the same reason. Also handles both wrapped (`{'data': [...], 'total': N}`) and bare-list association response shapes defensively.
- **Screening Recruiter-Activity Gate — Entity Association Note Lookup (Bug #5b, May 2026)**: `screening/dedup.py::_has_recent_recruiter_activity` previously shared the Bug #5 blind spot — it queried `search/Note?query=personReference.id:X AND dateAdded:[since TO *]` to detect recent human notes that should pause auto-vetting. Because that search index misses UI-added notes (linked via `candidates` association rather than `personReference`), the gate silently failed open and candidates with recent recruiter activity got re-screened. Concrete trigger: candidate 3849675 was re-screened at 4:28 PM despite a recruiter's General Note added at 10:19 AM the same day. Fix: switched to the same `entity/Candidate/{id}?fields=notes(id,dateAdded,commentingPerson(id))` association lookup used by owner_reassignment, with client-side filtering to the lookback window (entity endpoint has no server-side date filter) and newest-first iteration with early break on out-of-window notes. Handles both wrapped and bare-list association shapes; preserves existing single-retry, fail-open, and killswitch (`recruiter_activity_check_enabled`) semantics. Eliminates the parallel blind spot that allowed phantom re-screens of actively-worked candidates whose owner was still an API user.
- **Screening Recruiter-Activity Gate — Multi-API-User Exclusion (Bug #5c, May 2026)**: Bug #5b's fix introduced a regression. `_has_recent_recruiter_activity` was comparing the note author against only `bullhorn.user_id` (the single auth user, Myticas API User id=1147490) to decide "is this a human recruiter?" — but Bullhorn has SIX configured `api_user_ids` (Pandologic API, Myticas Parser, etc.). Notes authored by other API integrations (most importantly the `Pandologic API` "New application delivered" note attached to every PandoLogic-sourced candidate) were mis-classified as human recruiter activity, silently blocking every PandoLogic candidate from auto-vetting indefinitely. Concrete trigger: candidates 4658516 (Vivianne Grimoire), 4658519 (Samuel Rivas), 4658526 (Stephen Cain) — all PandoLogic-owned, all deferred for 23-97+ minutes with no human ever having touched them. Fix: `_is_paused_by_recruiter_activity` now reads `api_user_ids` from VettingConfig and passes it to `_has_recent_recruiter_activity`, which builds the union `{bullhorn.user_id} ∪ {configured api_user_ids}` and checks `int(cp_id) NOT in api_user_id_set` (matches the proven pattern in `tasks/owner_reassignment.py::_find_cooldown_busters_via_notes`). The auth user is always added defensively so back-compat is preserved when `api_user_ids` is empty/unset. New regression test suite (6 tests) covers PandoLogic case, multi-API-user batch, mixed human+API notes, empty/None api_user_ids back-compat, malformed entries, and wrapper-level VettingConfig integration.
