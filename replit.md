...
- **Scout Screening Quality Auditor**: Background AI audit that reviews **both** Not-Qualified results (false negatives) and a configurable sample of Qualified results (false positives), with auto-trigger re-vets on high-confidence findings. Three runtime knobs live in `VettingConfig` and surface in the Screening Quality Audit card on the settings page: `quality_auditor_model` (auditor-only model name, default `gpt-5.4`), `platform_age_ceilings` (JSON dict of platform → max-years used by the heuristic pre-checks), and `qualified_audit_sample_rate` (0-100 percent of Qualified results audited per cycle, default 10, 0 disables Phase 2). Loaders in `vetting_audit_service.py` (`get_auditor_model`, `get_platform_age_ceilings`, `get_qualified_sample_rate`) fall back to the in-file `DEFAULT_*` constants on missing/malformed config so seed/migration drift cannot break auditing. Per-candidate work is shared by `_process_candidate_audit(vetting_log, mode, summary)` which dispatches to `_run_heuristic_checks` (false-negative) or `_run_false_positive_checks` (false-positive: ≥2 mandatory-skill gap phrases on a Qualified score, negative-summary qualifiers at score ≥70%, or years_analysis showing <50% of required years while still marked meets_requirement). Locked by `tests/test_quality_auditor.py` (21 tests) and `tests/test_seed_vetting_config.py` EXPECTED_KEYS update.
...
- **Post-Merge Setup Hook**: `scripts/post-merge.sh` runs automatically after every background task-agent merge (registered via the `[postMerge]` section in `.replit`, 180s timeout). Performs `pip install -r requirements.txt` and `alembic upgrade head` so the running environment stays in sync with main when merges add Python dependencies or new Alembic revisions. Idempotent and non-interactive (`set -euo pipefail`).
- **Phone-Search Trigram Index**: Functional GIN trigram index `ix_parsed_email_phone_normalized_trgm` on `regexp_replace(parsed_email.candidate_phone, '[^0-9]', '', 'g')` (Alembic revision `0523f094143b`). Lets the recruiter Scout Screening phone substring lookup (introduced in #47) satisfy `ILIKE '%digits%'` against the normalized phone form via index scan instead of a sequential scan as `parsed_email` grows. Migration is PostgreSQL-only (no-ops on SQLite test dialect — the search query has its own SQLite fallback in `routes/scout_screening.py`), idempotent (`CREATE EXTENSION IF NOT EXISTS pg_trgm`, `CREATE INDEX IF NOT EXISTS`), and the downgrade leaves `pg_trgm` in place because other queries depend on it. Locked by `tests/test_phone_trigram_index_migration.py` (6 tests) covering revision wiring, single-head invariant, idempotent SQL emission, and SQLite no-op behavior.
