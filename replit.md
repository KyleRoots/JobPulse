...
- **Still pending**: (1) **48-72h cutover eval (~June 3-4)** against the 6-criterion playbooks (schema in `.local/tasks/output-token-diet.md`; L2 cache in `docs/archive-2026-05.md`) — output-token diet is the #1 cost lever; (2) **pricing-fix verify** — after republish, confirm new `openai_call_log` rows show ~$0.040/scoring-call and the dashboard matches the OpenAI bill ~1:1.
