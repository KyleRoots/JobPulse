...
**Task #98 L2 — screening.scoring cache cutover audit (Power, audit enabled 2026-05-15 PM)** — env flag only:
- `SCREENING_PROMPT_CACHE_AUDIT_ENABLED=true` set in Replit deployment secrets. Shadow comparison rows now accumulating in `screening_ab_log` tagged `{model}|legacy` vs `{model}|cache_optimized`. Prod scoring unaffected (`SCREENING_PROMPT_LAYOUT` left at default `legacy`).
