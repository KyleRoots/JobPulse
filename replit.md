...
- **Bullhorn Search API stale-index pattern (NEW 2026-05-15)**: Bullhorn's Search API has a recurring lag vs. its Entity API on tearsheet membership — when a job goes `isOpen=False` or is removed via DELETE, Entity API updates immediately but Search API can serve the stale row for ~5-6 min (sometimes longer). Surfaced today on tearsheet 1231 (jobs 34629 + 34952 — resolved by manual recruiter removal at ~12:10 UTC; Search API caught up by 12:16); currently still active on tearsheet 1531 STSI (`Entity API reports 15 jobs but Search API returned 16` — 1 ghost). Our auto-removal handles closed jobs correctly when the row stays in Search; manual removal handles edge cases. **Not a defect on our side**, but two operational side effects worth knowing: (1) brief feed discrepancy with the Active Monitors UI count until next cycle; (2) for any zombie job in Search, our screening pipeline re-extracts requirements every cycle (~$2-4/day per zombie at gpt-5.4 rates). **Future task candidate** (Economy-tier): defensive cooldown in `incremental_monitoring_service` to skip BOTH DELETE retry + AI requirements re-extraction for jobs auto-removed in the last N hours, plus a Sentry alert for "auto-removal repeat offenders" ≥5 cycles. Hygiene fix for STSI: manually inspect tearsheet 1531 in Bullhorn for any closed/ineligible job and remove it.
