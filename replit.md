...
**Reference Number Flip-Flopping Bug COMPLETELY FIXED**: Resolved critical issue where reference numbers were changing every 5 minutes between two different values. ROOT CAUSE: The comprehensive monitoring service was uploading TWO conflicting files per cycle - main file (correct) + scheduled version file (containing old data from July 23rd). SOLUTION: Disabled scheduled version upload in monitoring service and eliminated snapshot system. The system now preserves reference numbers during all monitoring cycles and only changes them during manual "Refresh All" operations. VERIFICATION: Live website and local files maintain identical reference numbers after monitoring cycles.
