...
- **Magic Byte File Format Detection**: `vetting/resume_utils.py` inspects actual file content bytes (not filename extension) to determine real format: `PK` = DOCX/ZIP, `\xD0\xCF\x11\xE0` = legacy DOC, `%PDF` = PDF. When extension-based extraction fails, the system automatically retries with the correct extractor for the detected format, then falls back to LibreOffice conversion or AI vision OCR as a last resort.
- **AI Job Classification**: Classifies jobs based on LinkedIn taxonomy.
- **Scout Vetting**: AI-powered candidate screening using GPT-5.4 with embedding pre-filtering, experience-level classification, two-phase scoring, work authorization/security clearance inference, configurable global screening prompts, employment gap penalties, AI-native recency relevance enforcement with strict justification requirements, and per-job Employer Prestige Boost (+5 pts for candidates at major consulting firms like Big 4, Accenture, McKinsey, etc. — requires location compatibility). Prestige detection uses word-boundary regex matching with display-name normalization. Below-threshold prestige candidates trigger special amber recruiter notification emails. Toggle in per-job Configure modal on Scout Screening portal.
...
- **Scout Support**: AI-powered internal ATS support ticket module with two-tier approval workflow, AI intake, clarification loops, solution proposals, and Bullhorn API execution. Includes Knowledge Hub for self-learning via uploaded documents (PDF, DOCX, TXT) and OneDrive integration for document synchronization. **Dual feedback loop**: (1) Positive — successfully completed tickets auto-generate `resolution_pattern` knowledge entries via `learn_from_ticket()`; (2) Negative — escalated/failed tickets auto-generate `escalation_pattern` knowledge entries via `learn_from_escalation()` with AI-powered failure analysis (gpt-4.1-mini) that extracts lessons learned. Knowledge context injection distinguishes failure patterns (priority warning) from success patterns (proven approaches). Admin conversation takeover: when ticket is escalated, admin can reply directly to user from ticket detail page or via email; ticket moves to `admin_handling` status and AI steps aside. User replies during admin handling are forwarded to admin. Closed ticket reopening: user replies to closed/completed tickets trigger AI re-analysis with full conversation/execution history; AI handles directly if possible, proposes new solution if needed, or re-escalates. Direct escalation keywords bypass AI. Intelligent retry with contextual notifications: retry emails include failure analysis and new strategy summary. Attachment persistence: uploaded screenshots/files stored in `SupportAttachment` model (binary in DB) with `conversation_id` FK, displayed as thumbnails/download links on both admin and user ticket detail pages; admin replies support file attachments (max 10MB, PNG/JPG/GIF/WEBP/PDF/DOCX/DOC/TXT). Admin AI drafting: during `admin_handling`, admin email replies are classified as either direct user messages or AI instructions; AI instructions trigger GPT-5 to generate requested content (e.g., draft emails) and send back to admin only, without forwarding to the user.
