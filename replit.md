...
### Active — Closing Batch (2026-06-04, pending one deploy + verify)
- **Mailbox-Pull emergency — RESOLVED**: GCE load-balancer body truncation + the Graph `$select` attachment bug are both fixed and the manual "Recover Missing Résumés" recovery was run. Full incident history archived in `docs/archive-2026-06.md`. Steady mailbox polling resumes when `mailbox_pull_enabled=true`.
- **Two-column résumé extraction (Item A) — SHIPPED, pending deploy verify**: PDF parsing in `resume_parser.py` is now column-aware (`_extract_pdf_page_text`) — detects a genuine two-column layout (central gutter dead-band, ≥3 blocks each side, columns carry the bulk of text mass) and reads header → left column → right column → footer, so sidebars (skills/contact) are no longer interleaved or lost. Single-column résumés are untouched (guards reject right-aligned dates / banners). DOCX already pulls text-box content. Covered by `tests/test_pdf_column_extraction.py` (5 tests). **Post-deploy verify**: spot-check a real two-column PDF applicant parses full sidebar+body; confirm no single-column regression.
- **Auto-recovery sweep (Item B) — SHIPPED, pending deploy verify**: the manual recovery engine is now also a scheduled sweep ("Résumé Recovery Sweep (30 min)" in `scheduler_setup.py`) that auto-detects completed-but-no-résumé rows and re-attaches to the EXISTING candidate — idempotent + single-flight, never creates a candidate/submission. Gated by DB flags `resume_recovery_sweep_enabled` (default ON), `resume_recovery_sweep_window_hours` (default 6), `resume_recovery_sweep_limit` (default 25); runs independent of `mailbox_pull_enabled` so it heals even while polling is paused. **Post-deploy verify**: confirm the 30-min job logs a sweep, recovers any eligible rows, and reports 0 on a clean second pass.
- **Clearance wording (Item 4) — SHIPPED (seed only)**: `config/global_screening_prompt.txt` clearance wording aligned; thresholds confirmed consistent 5/10/15 (Reliability/Secret/TopSecret, Canadian). Live value is the DB (`VettingConfig.global_custom_requirements`); the file is seed only, so prod copy needs the user to re-apply if they want it live.
...
- **Steady-state run-rate (2026-06-02, audits OFF)**: all-in billing-true cost ≈ **$0.06 / candidate-job match** (scoring ≈1.35 calls/match @ ~$0.044 billing-true + ~7% non-scoring overhead). Natural volume (clean May 1-10 window, excl. Monday outlier) ≈ **1,800-2,200 matches/day** → **~$110-140/day → ~$3,400-4,200/mo (~$3,800 typical)** for the OpenAI bill. Volume-proportional. Below the original ~$4,700/mo baseline; above the artificially-low ~$2,300 May figure (screening was OFF half of May). Transient backlog catch-up (~3,400 candidates after the May 11–Jun 1 outage) adds a one-time bump for ~1 week, then settles.
...
### Active — Operational
- **Dynamic source attribution (apply page) — SHIPPED, pending deploy verify**: detects the TRUE application channel on OUR side instead of trusting the vendor param. Every published apply URL hardcodes `?source=LinkedIn` (`xml_integration_service/mapping.py`), so that param is a static default, not a per-vendor signal. The apply-page GET now captures the browser referrer + `?source=`/`utm_*` (first-touch logged to new `apply_page_visit` table), carries a token + referrer/utm via hidden form fields, and at submit time resolves a canonical Bullhorn source (`source_attribution.resolve_source`: referrer host > utm > param) and stamps it into the "has applied on {source}" email subject — so the existing inbound→Bullhorn pipeline carries the real channel untouched. Internal + PandoLogic referrers deliberately fall through (Pando masks origin). Inbound matcher is now tolerant ("Indeed"→"Indeed Job Board"). `&feed=pando` ownership routing unchanged. All attribution reads/writes fail-soft. Covered by `tests/test_source_attribution.py` (8 tests). **Post-deploy verify**: confirm `apply_page_visit` rows accumulate with non-LinkedIn `resolved_source` for genuine Indeed/Dice/etc. arrivals, and the matching Bullhorn submissions show the real source; spot-check no regression where referrer is stripped (still defaults LinkedIn). **Deferred (not a defect)**: define a retention/TTL policy for `apply_page_visit` (stores IP/UA/email/referrer).
