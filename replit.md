...
- **Automation Hub (Super-Admin Only)**: Management console at `/automations` replacing the former Product Expert Workbench. Three sections: (1) Scheduled Jobs — live grid of APScheduler jobs with status dots, next/last run times, and pause/resume controls; (2) Built-in Tools — 8 seeded automation cards (cleanup_ai_notes, cleanup_duplicate_notes, find_zero_match, export_qualified, resume_reparser, salesrep_sync, update_field_bulk, email_extractor) with parameter modals and dry-run toggle; (3) Run History — paginated AutomationLog table. No AI chat layer — purely a management and execution console. Long-running builtins execute in background threads with polling-based status updates. Blueprint: `routes/automations.py`, template: `templates/automations.html`, service: `automation_service.py`.
...
- **Activity Log (Super-Admin Only)**: System-wide admin visibility page at `/activity-log` under the System sidebar section. Four tabbed views: Login History (tracks every login with user, timestamp, IP, browser/OS), Module Usage (session-based tracking of which modules users navigate to — deduplicates within a session), Email Delivery (unified view of welcome, password reset, and screening recommendation emails from `EmailDeliveryLog`), and Active Users (real-time view of currently active users with status dots: green <5min, orange 5-15min, grey 15-60min, auto-refreshes every 30s). Summary metric cards show 7-day totals. Filterable by user and date range (7/30/90 days). Blueprint: `routes/activity_log.py`, template: `templates/activity_log.html`. DB model: `UserActivityLog` (user_id, activity_type, details JSON, ip_address, created_at). `User.last_active_at` column tracks real-time activity (throttled to 1-minute writes via `before_request` hook). Login tracking hooks into `routes/auth.py` login route. Module tracking via `@app.before_request` hook with `_MODULE_MAP` prefix matching. Welcome and password reset emails now logged to `EmailDeliveryLog` (types: `welcome_email`, `password_reset_email`).
