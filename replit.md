...
- **AI Fuzzy Duplicate Matcher**: Two-pass dedup cycle. Pass 1 keeps the existing exact email/phone matcher; Pass 2 catches duplicates where BOTH email and phone changed. Builds a profile string (name + work history + skills + location + education — phone/email deliberately omitted), embeds via OpenAI `text-embedding-3-large`, caches the embedding in `CandidateProfileEmbedding` keyed by SHA-256 of the profile text (auto-invalidates on profile change), runs a numpy cosine pre-filter (top-N=5, threshold 0.75), then asks GPT-5.4 for a final confidence score — merges only fire at ≥0.90. Per-cycle cap of 25 candidates with a persistent `FuzzyEvaluationQueue` overflow table (FIFO drain, hard ceiling of 24 attempts so broken records can't starve the tail). Cold-start handled by a cursor-driven backfill (`fuzzy_dedup_backfill_cursor_id` in `GlobalSettings`) that embeds 50 historical candidates per cycle and walks the corpus via `id > cursor`. `CandidateMergeLog.match_type` distinguishes `'exact'` vs `'ai_fuzzy'` merges. Operational WARNINGs fire when fuzzy errors > 0 in a cycle or queue depth > 250.
