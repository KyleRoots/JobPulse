...
def run_schema_migrations(db):
    ...
    try:
        ...
        if not has_uq:
            ...
            """))
            db.session.commit()
            removed = deleted.rowcount if deleted.rowcount is not None else 0
            if removed:
                logger.info(
                    f"✅ Deduped global_settings: removed {removed} duplicate setting_key row(s)"
                )
            db.session.execute(text("SET LOCAL lock_timeout = '5s'"))
            db.session.execute(text(
                f'CREATE UNIQUE INDEX IF NOT EXISTS {_gs_uq} '
                f'ON global_settings (setting_key)'
            ))
            db.session.commit()
...
def migrate_qualified_audit_sample_rate_to_zero(db):
    """
    Retired one-time migration (May 2026 S6).

    The original migration force-flipped ``qualified_audit_sample_rate``
    from the legacy default '10' down to '0'. That work is complete on
    every environment. Retained as a no-op so existing seed_database.py
    call sites keep working without import errors. The value is now
    administrator-owned via ``VettingConfig`` / `/vetting/settings` and
