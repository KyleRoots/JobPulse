...
## 2026-06-09 trim — cheap-first routing rollout, Enforce checkpoint, and verified prod items

### Mini-model cheap-first routing — full rollout play-by-play (CONCLUDED; Enforce baked in for July re-enable)
- **Design/validation**: validated on the gpt-4.1-mini A/B shadow (6,302 head-to-head scorings vs gpt-5.4 since 2026-05-14): **97.8% qualify/reject agreement**, mini +8.5 generous (rarely false-rejects), base rate ~0.7% qualify (43/6,302). Cheap-first gate (mini scores everyone; mini < 40 → auto-reject/skip gpt-5.4; else escalate to gpt-5.4 as authoritative scorer) routes **~74.5% cheap with 0/43 qualified lost** → ~62% off the scoring line → **~$2,000–2,600/mo ongoing**. Full design in `.agents/memory/cheaper-model-scoring-experiment.md`.
- **Implementation**: VettingConfig DB keys (per-environment): `layer2_model` (`gpt-4.1-mini` turns mini on as first-pass; `gpt-5.4` makes the router a hard no-op), `screening_routing_mode` (`off` | `canary` = gate logged, gpt-5.4 still runs on everyone, $0 savings | `enforce` = mini<thr skips gpt-5.4), `cheap_first_reject_threshold` (default 40). gpt-5.4 stays the SOLE qualification authority — even an escalation infra failure (canary/enforce re-raise → job recorded 0/error → eligible for the zero-score gpt-5.4 reverify; never qualifies on mini alone). Set via the `/screening` Routing Mode dropdown. Tests: `tests/test_cheap_first_routing.py` (13).
- **Rollout timeline**: Canary LIVE in prod 2026-06-05 ~19:11 UTC (`🧭 Cheap-first routing: mode=canary`, 0 false-negatives). Enforce LIVE 2026-06-05 ~20:38 UTC.
- **Mini numeric-field crash — fixed in two passes (both deployed + verified prod 2026-06-05)**: mini sometimes returns numeric fields as prose ("Not explicitly stated…") or explicit JSON null where gpt-5.4 always sent numbers → `float()`/`int()`/`>` crashed analysis → match_score 0 (only after canary went live). Pass 1: years fields hardened via `_safe_float` in `post_processing.py` + re-check guard in `prompt_builder.py`; unparseable estimate skips that skill's years gate. Pass 2: hardened `enforce_recency_hard_gate` and `coerce_scores` (`match_score`/`technical_score` → `int(_safe_float(...))`). Root trap: `dict.get(k, 0)` returns None on an EXPLICIT null (default only covers a MISSING key). Tests: `tests/test_years_parsing_hardening.py` (11) + `tests/test_recency_coerce_hardening.py` (8). Lesson in `.agents/memory/mini-numeric-field-coercion.md`. Separate observed (left unfixed on purpose): mini occasionally emits truncated JSON (`Unterminated string`) — caught → escalates to gpt-5.4, no candidate lost.
- **Known pre-existing follow-up (out of scope)**: zero-score reverify recomputes `is_qualified` without the location-barrier guard (`processing.py` ~750) — architect-confirmed pre-existing.

### Jun 8–9 Enforce-validation checkpoint (CONCLUDED, overtaken by the June budget pause)
- Enforce LIVE since 2026-06-05 ~20:38 UTC. Interim read 2026-06-07 = healthy — scoring saving **~$94/day (~$2,800/mo)** vs all-gpt-5.4, qualify rate holding (~1%/day, notifications flowing), **0** `🚨 CANARY false-negative` + 0 `AI analysis error`/numeric-crash lines, cheap-reject path firing (`💸 Cheap-first reject`).
- (1) A/B shadow RETIRED ✅ 2026-06-08 (off, verified $0/day) — recovered ~$1,950/mo monitoring overhead (measured ~$65/day on `screening.scoring.shadow`). NOTE: the shadow was the SOLE false-negative detector in Enforce, so it was retired only after the gate confirmed clean.
- (2)–(4) require live scoring → deferred to the 2026-07-01 re-enable: confirm 0 false-negative + qualify-rate parity across the full window; re-measure scoring steady-state; skip rate was **~46%** (below the ~74.5% design target) because a few mid-band candidates are re-scored repeatedly → all escalate → reinforces the Re-vet dedupe opportunity.

### Verified prod items (closed 2026-06-07 unless noted)
- **Dynamic source attribution — deployed 2026-06-05, VERIFIED 2026-06-07**: apply page detects the true channel via the browser referrer (first-touch `apply_page_visit`) instead of the hardcoded `?source=LinkedIn`; resolves referrer > utm > param and stamps the "has applied on {source}" email subject → inbound→Bullhorn carries it untouched. Prod evidence: two real Facebook arrivals (referrer_host `l.facebook.com`/`facebook.com`) resolved to `Facebook` despite carrying `source_param=LinkedIn`; breakdown 609 LinkedIn / 63 blank (internal/PandoLogic fall-through) / 2 Facebook. `tests/test_source_attribution.py` (8). Deferred: retention/TTL for `apply_page_visit` (stores IP/UA/email/referrer). NOTE: PandoLogic referrers fall through here — superseded by the feed=pando attribution shipped 2026-06-09 (tracked in replit.md until its pando-path verify closes).
