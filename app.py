...
def process_bullhorn_monitors():
    ...
    with app.app_context():
        try:
            ...
            for monitor in due_monitors:
                ...
                try:
                    ...
                    if added_jobs or removed_jobs or modified_jobs:
                        ...
                        xml_files_to_update = ['myticas-job-feed.xml']  # Back to standard file name
                        active_schedules = ScheduleConfig.query.filter_by(is_active=True).all()
            ...
            # CRITICAL FIX: Only run comprehensive sync if there are actual changes detected
            # DO NOT run comprehensive sync just to add "missing" jobs - this causes bulk reference regeneration
            # Comprehensive sync should only handle job modifications, not bulk additions
            ...
            if monitors_processed and len(monitors_processed) > 0:
                ...
                if total_jobs > 0 or (total_jobs == 0 and is_system_wide_sync):
                    # CRITICAL FIX: Use proper ComprehensiveMonitoringService with reference number preservation
                    app.logger.info("🔄 USING ComprehensiveMonitoringService with reference number preservation")
                    ...
                    try:
                        ...
                        comprehensive_sync_made_changes = cycle_results.get('jobs_added', 0) > 0 or cycle_results.get('jobs_removed', 0) > 0 or cycle_results.get('jobs_modified', 0) > 0
                        comprehensive_sync_summary = {
                            'added_count': cycle_results.get('jobs_added', 0),
                            'updated_count': cycle_results.get('jobs_modified', 0), 
                            'removed_count': cycle_results.get('jobs_removed', 0)
                        }
                        
                        app.logger.info(f"✅ ComprehensiveMonitoringService completed: {comprehensive_sync_summary}")
                        
                    except Exception as comprehensive_error:
                        app.logger.error(f"❌ ComprehensiveMonitoringService failed: {str(comprehensive_error)}")
                        # Fallback to existing logic
                        comprehensive_sync_made_changes = False
                        comprehensive_sync_summary = {'added_count': 0, 'updated_count': 0, 'removed_count': 0}

                else:
                    app.logger.info("⏭️ SKIPPING COMPREHENSIVE SYNC: No monitors processed or no jobs detected")
                    comprehensive_sync_made_changes = False
                    comprehensive_sync_summary = {'added_count': 0, 'updated_count': 0, 'removed_count': 0}

        # Continue to Steps 5-8 of Enhanced 8-Step Monitor (formatting, upload, audit)
        # These steps are handled by the existing Enhanced Monitor workflow below
...
app.logger.info("Using ENHANCED 8-step comprehensive monitoring process with ComprehensiveMonitoringService integration")
