...
## Zero-score reverify = PARTIAL Enforce safety net (verified)
In Enforce a mini-rejected candidate is recorded at its mini score; the zero-score reverify (`screening.zero_recheck`) re-scores with gpt-5.4 and is authoritative — it resolves model via `resolve_model('screening.zero_recheck','gpt-5.4')` (env-override-only, NOT the `layer2_model` DB key) and calls gpt-5.4 DIRECTLY, bypassing the cheap-first router, so it stays gpt-5.4 regardless of routing mode. **BUT it is conditional:** it only fires when the candidate scored **0 on ALL matched jobs** (`all(m.match_score==0)`) and only rechecks the **top 3**. So it rescues the common catastrophic miss (a real fit mini zeroed across the board, esp. single-job applicants) but does NOT rescue a candidate mini-scored 1–39 (rejected but non-zero) on their fit job, or with mixed/non-zero scores elsewhere. Real backstop, not a force field. **Why it matters:** this is the same re-vet redundancy we want to dedupe — keep a gpt-5.4 zero-recheck path alive when trimming re-vets, and to strengthen it make it fire per-rejected-job (not just all-zero).

## Savings sizing — surge vs steady-state (don't conflate)
Scoring (`screening.scoring`, gpt-5.4) is ~82% of the entire OpenAI bill; mini is trivially cheap. Enforce cuts ~62% of the scoring line. **Two very different daily numbers depending on volume:** during a post-outage backlog SURGE (~3× normal) the cut is large in absolute $ (~$165–195/day telemetry ≈ ~47% of daily burn) but burn still exceeds a ~$2,500/mo cap; the cap is only met once the backlog DRAINS to steady-state (~1/3 volume), where Enforce yields the durable **~$1,700–2,600/mo**. Telemetry runs ~1.2–1.6× below the real OpenAI bill, so real-$ savings are higher than telemetry shows.
