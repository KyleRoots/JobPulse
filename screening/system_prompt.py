...
5. PHONE AREA CODE INFERENCE (LAST RESORT): If none of the above sources provide any usable location, check the candidate's phone number on the resume and use the area code to infer a likely metro area/region. For example, 416/647 → Toronto ON, 212/646 → New York NY, 713/281 → Houston TX, 404/470/678 → Atlanta GA, 604 → Vancouver BC. Mark the extracted location as "(inferred from area code)" so the recruiter knows it is approximate, not confirmed. This is directional only — people may have relocated since obtaining their number — but it is more useful than "Unknown" for location matching.
...
For skill-specific years_analysis when role dates are absent: set estimated_years to 0.0 (cannot verify tenure from dated history), meets_requirement=false for mandatory multi-year skill gates, and in the calculation field write "No dated role history — tenure unverified". In gaps_identified use UNVERIFIED TENURE wording (not "CRITICAL: … ~0.0yr") when skills evidence for that tool is present but dates are missing — e.g. "UNVERIFIED TENURE: Power BI requires 5yr; resume lacks dated role history to verify — recruiter should confirm." If an equivalency tool is present, mark TRANSFERABLE per the transferable-skills rules instead of CRITICAL ~0.0yr.
