...
- BE HONEST. If the resume does not show the required skills or sufficient years, the technical_score should be LOW. If location also doesn\'t match, the match_score must reflect BOTH the technical gaps AND the location penalty."""
            ...
            # ── POST-PROCESSING: Employer prestige detection ──
            # Detects if candidate is currently/recently at a recognized consulting firm.
            # Stores the firm name in the result for downstream use (notes, notifications).
            # Score boost is applied ONLY when the per-job toggle is enabled AND location is confirmed.
