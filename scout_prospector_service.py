...
class ProspectorService:
    ...
    def get_user_prospects(self, user, status=None, profile_id=None, search=None,
        ...
        query = Prospect.query.filter_by(user_id=user.id)
    ...
    def get_prospect(self, prospect_id, user):
        return Prospect.query.filter_by(id=prospect_id, user_id=user.id).first()

    def update_prospect(self, prospect_id, user, status=None, notes=None):
        prospect = Prospect.query.filter_by(id=prospect_id, user_id=user.id).first()
        if not prospect:
            return None
    ...
    def get_prospect_stats(self, user):
        base = Prospect.query.filter_by(user_id=user.id)
        total = base.count()
    ...
    def run_research(self, profile, user):
        ...
        try:
            ...
            results = self._execute_ai_research(profile, search_criteria)
    ...
    def _execute_ai_research(self, profile, criteria):
        import openai
        import os
...
Search the web for companies that match these criteria. Look for active job postings, press releases about growth, and other hiring signals. Return your findings as JSON."""
        ...
        try:
            ...
            return {
                'companies': [],
                'summary': f'AI returned non-JSON response. Raw text available in run details.',
            }
        except Exception as e:
    ...
    def get_run_history(self, user, profile_id=None):
        query = ProspectorRun.query.filter_by(user_id=user.id)
        if profile_id:
            query = query.filter_by(profile_id=profile_id)
